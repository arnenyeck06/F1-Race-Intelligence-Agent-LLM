...
PROMPT_A = """You are an F1 Race Intelligence assistant.
Answer the question based ONLY on the provided race data excerpts.
If the excerpts don't contain enough information, say so clearly."""

PROMPT_B = """You are an F1 Race Intelligence assistant specializing in race analysis.
Think step by step:
...
Always cite: source and year for each fact you use.
If the excerpts don't contain enough information, say so explicitly."""

JUDGE_PROMPT = """You are evaluating an F1 RAG system answer.
...
def main():
    ...
    index = Index(text_fields=["text"], keyword_fields=["category", "source"])
    index.fit(chunks)
    embedder = Embedder()
    ...
    for i, record in enumerate(sample):
        print(f"[{i+1}/{len(sample)}] {record['source'][:50]}")
        retrieved = hybrid_search(query=record["question"], index=index, embedder=embedder, num_results=5)
