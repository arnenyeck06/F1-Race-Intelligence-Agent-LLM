...
def hybrid_search(query, index, embedder, category=None, num_results=5, k=60):
    keyword_results = index.search(query, num_results=num_results * 2)
    q_vec = embedder.encode(query)
    vector_results = pgvector_search(q_vec, category=category, num_results=num_results * 2)
